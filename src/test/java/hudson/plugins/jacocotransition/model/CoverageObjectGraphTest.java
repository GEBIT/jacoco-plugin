...
import hudson.plugins.jacocotransition.AbstractJacocoTestBase;
...
public class CoverageObjectGraphTest extends AbstractJacocoTestBase
{
	...
	private void assertGraph(JFreeChart chart, String file, boolean writeFile) throws IOException
	{
		...
		try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			if(!(new File(TEST_RESOURCES + file+".old")).exists()){
				FileUtils.copyFile(new File(TEST_RESOURCES + file), new File(TEST_RESOURCES + file+".old"));
			}
			FileUtils.writeByteArrayToFile(new File(TEST_RESOURCES + file), actual);
		}
		try
		{
