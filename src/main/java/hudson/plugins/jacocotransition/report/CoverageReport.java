...
import hudson.plugins.jacocotransition.annotation.PackageCoverageMapper;
...
public final class CoverageReport extends AggregatedReport<CoverageReport/*dummy*/,CoverageReport,PackageReport> {
	private final JacocoIntegrityTrendTransitionBuildAction action;

	private CoverageReport(JacocoIntegrityTrendTransitionBuildAction action) {
		this.action = action;
		setName("TrendIntegrityJacoco");
	}
...
//	private String methodColor;
	...
	public CoverageReport(JacocoIntegrityTrendTransitionBuildAction action, @Nonnull ExecutionFileLoader executionFileLoader ) {
		...
		if (executionFileLoader.getBundleCoverage() !=null ) {
			...
			for (IPackageCoverage packageCov: packageList) {
				...
				packageReport.setWorkflowType(((PackageCoverageMapper)packageCov).getWorkflowType());
				this.setCoverage(packageReport, packageCov);
				//System.err.println("><"+packageCov.getName());
				...
				for (IClassCoverage classCov: classList) {
					...
					//System.err.println("><"+packageCov.getName()+"><"+classCov.getName());
	...
	protected void printRatioTable(Coverage ratio, StringBuilder buf){
		...
		.append("<div class='percentgraph' style='width:100px; background-color:")
		.append(ratio.getCoveredStyle())
		.append(";'>")
		.append("<div class='percentgraph' style='width:")
		.append(100 - ratio.getPercentage()).append("px; background-color:")
	...
	//@Override
	protected void printRatioTableO(Coverage ratio, StringBuilder buf){
		buf.append("<table class='percentgraph' cellpadding='0' cellspacing='0'><tr class='percentgraph'>")
		.append("<td style='width:40px' class='data'>").append(ratio.getPercentage()).append("%</td>")
		.append("<td class='percentgraph'>")
	...
	public CoverageReport getPreviousResult() {
		JacocoIntegrityTrendTransitionBuildAction prev = action.getPreviousResult();
		if(prev!=null) {
