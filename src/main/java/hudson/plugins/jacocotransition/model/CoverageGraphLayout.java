...
import hudson.plugins.jacocotransition.Messages;
...
public class CoverageGraphLayout
{
	...
	public CoverageGraphLayout reportType(ReportType reportType) {
		CoverageType type = reportType.getJacocoCoverageType();
		Color missed = reportType.getMissed();
		Color covered = reportType.getCovered();
		
		plot().type(type).value(CoverageValue.MISSED).color(missed).plot().type(type).value(CoverageValue.COVERED).color(covered);
		return this;	
	}
